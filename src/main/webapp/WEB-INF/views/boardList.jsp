    ...
    <div class="board-container">
        ...
        <table>
            ...
            <c:forEach var="boardDto" items="${list}">
                <tr>
                    ...
                    <td class="title"><a href="http://localhost:8080/board/read?bno=${boardDto.bno}&page=${list.number}">${boardDto.title}</a></td>
        ...
        <div class="paging-container">
            <div class="paging">
                ...
                <c:if test="${totalCnt!=null && totalCnt!=0}">
                    <c:if test="${showPrev}">
                        <a class="page" href="http://localhost:8080/board/list?page=${beginPage - 1}"/>&lt;</a>
                    </c:if>
                    <c:forEach var="i" begin="${beginPage}" end="${endPage}">
                        <a class="page ${i==ph.sc.page? "paging-active" : ""}" href="http://localhost:8080/board/list?page=${i-1}"/>${i}</a>
                    </c:forEach>
                    <c:if test="${showNext}">
                        <a class="page" href="http://localhost:8080/board/list?page=${endPage + 1}"/>&gt;</a>
                    </c:if>
                </c:if>
            </div>
        </div>
